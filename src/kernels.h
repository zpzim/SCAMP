...
__global__ void do_tile_self_join_upper(const DTYPE_IN* QT, const DTYPE_IN *T_a, const DTYPE_IN *T_b, const DTYPE_IN *inv_stds_A, const DTYPE_IN *inv_stds_B, const DTYPE_IN *means_A,
                                        const DTYPE_IN *means_B, unsigned long long int *profile_A, unsigned long long int *profile_B, unsigned int m, unsigned int n,
                                     ...
                                     unsigned long long int *profile_B, size_t window_size, size_t tile_width, cudaStream_t s)
{
        ...
        do_tile_self_join_upper<DATATYPE, float, BLOCKSZ, UNROLL_COUNT><<<dim3(ceil(tile_width / (double) BLOCKSZ), 1, 1), dim3(BLOCKSZ, 1,1), 0, s>>>(QT, timeseries_A, timeseries_B, means_A, means_B, std_dev_A, std_dev_B, profile_A, profile_B, window_size, tile_width, reg_mem<UNROLL_COUNT>());
