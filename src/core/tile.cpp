...
std::pair<int, int> Tile::get_exclusion_for_self_join(bool upper_tile) {
  ...
  // The main diagonal is handled by the upper tile. Which means that we need to
  // apply one less diagonal to the exclusion zone for lower tiles.
  // Transposed tiles are always off the main diag. Since the main diag counts
  // as the first excluded diagonal (i.e. in an exclusion zone of 1 we only get
  // rid of the main diagonal.) we need to reduce the amount excluded from
...
std::pair<int, int> Tile::get_exclusion_for_ab_join(bool upper_tile) {
  ...
  if (upper_tile) {
    ...
    // The main diagonal is handled by the upper tile. Which means that we need
    // to apply one less diagonal to the exclusion zone for lower tiles.
    // Transposed tiles are always off the main diag. Since the main diag counts
    // as the first excluded diagonal (i.e. in an exclusion zone of 1 we only
    // get rid of the main diagonal.) we need to reduce the amount excluded from
    // transposed tiles to account for this.
    //
    //
    //               A A A A T   Legend: A = normal tile, T = Transposed tile
