...
void __device__ do_iteration_fast(const SCAMPKernelInputArgs<double>& args,
  ...
  if (info.global_row == 0) {
    info.dfc = Eigen::Map<const Eigen::Array<double, inner_unrolled_cols, 1>>(
                   args.dfa + info.global_col)
                   .template cast<DerivedDataType>();
    info.dgc = Eigen::Map<const Eigen::Array<double, inner_unrolled_cols, 1>>(
                   args.dga + info.global_col)
                   .template cast<DerivedDataType>();
    info.inormc =
        Eigen::Map<const Eigen::Array<double, inner_unrolled_cols, 1>>(
  ...
  for_<outer_unrolled_rows / unrolled_rows>([&](auto j) {
    if (info.global_row != 0 || j.value > 0) {
      auto temp = ConvertToIntrinsic<DerivedDataType, unrolled_rows>(
      ...
      temp = ConvertToIntrinsic<DerivedDataType, unrolled_rows>(
      ...
      if (this_warp.thread_rank() == 31) {
        info.dfc.segment<unrolled_rows>(self_overlap) =
            Eigen::Map<const Eigen::Array<double, unrolled_rows, 1>>(
                args.dfa + info.global_col + j.value * unrolled_rows +
                self_overlap)
                .template cast<DerivedDataType>();
        info.dgc.segment<unrolled_rows>(self_overlap) =
            Eigen::Map<const Eigen::Array<double, unrolled_rows, 1>>(
                args.dga + info.global_col + j.value * unrolled_rows +
                self_overlap)
                .template cast<DerivedDataType>();
        info.inormc.segment<unrolled_rows>(self_overlap) =
            Eigen::Map<const Eigen::Array<double, unrolled_rows, 1>>(
                args.normsa + info.global_col + j.value * unrolled_rows +
                self_overlap)
                .template cast<DerivedDataType>();
      }
    }
