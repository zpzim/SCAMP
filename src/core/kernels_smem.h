...
template <typename SMEM_TYPE, bool COMPUTE_ROWS, bool COMPUTE_COLS, int tile_width, int tile_height, int BLOCKSZ>
...
template <typename SMEM_TYPE, typename PROFILE_DATA_TYPE, typename PROFILE_OUTPUT_TYPE, bool COMPUTE_ROWS, bool COMPUTE_COLS, int tile_width, int tile_height, int BLOCKSZ, int PROFILE_TYPE>
__device__ void init_smem(SCAMPKernelInputArgs<double> &args,
                          ...
                          uint32_t row_start) {
  if constexpr (PROFILE_TYPE == PROFILE_TYPE_1NN_INDEX || PROFILE_TYPE == PROFILE_TYPE_1NN) {
    init_smem_with_dynamic_initializer<SMEM_TYPE, PROFILE_DATA_TYPE, COMPUTE_ROWS,
...
template<SCAMPProfileType PROFILE_TYPE, typename DerivedProfile, typename DerivedSmem> 
__device__ void write_back_value(SCAMPKernelInputArgs<double> &args, int local_position, int global_position, const Eigen::ArrayBase<DerivedSmem>& smem_profile, DerivedProfile *profile, uint64_t *profile_length, const float *thresholds) {
  if constexpr (PROFILE_TYPE == PROFILE_TYPE_1NN) {
      fAtomicMax<ATOMIC_GLOBAL>(profile + global_position, smem_profile[local_position]);
    ...
    do_atomicAdd<DerivedProfile, ATOMIC_GLOBAL>(
          profile + global_position, smem_profile[local_position]);
...
__device__ void write_back(SCAMPKernelInputArgs<double> &args,
  ...
  if constexpr (COMPUTE_COLS) {
    ...
    while (local_position < TILE_WIDTH && global_position < n_x) {
      write_back_value<PROFILE_TYPE>(args, local_position, global_position, smem.local_mp_col, profile_A, smem.profile_a_length, args.thresholds_a);
      global_position += BLOCKSZ;
      local_position += BLOCKSZ;
    }
  }
  if constexpr (COMPUTE_ROWS) {
    ...
    while (local_position < TILE_HEIGHT && global_position < n_y) {
      write_back_value<PROFILE_TYPE>(args, local_position, global_position, smem.local_mp_row, profile_B, smem.profile_b_length, args.thresholds_b);
      global_position += BLOCKSZ;
      local_position += BLOCKSZ;
    }
  }
