...
helloworld::SCAMPProfileType profile_type = helloworld::PROFILE_TYPE_1NN_INDEX;
helloworld::SCAMPPrecisionType precision_type = helloworld::PRECISION_DOUBLE;
...
void MergeProfile(Profile *profile_a, Profile *a_tile, uint64_t col_pos,
                  uint64_t width, std::mutex &a_lock, Profile *profile_b,
  ...
  // Merge result
  MergeTileIntoFullProfile(a_tile, col_pos, width, profile_a, row_pos, a_lock);
...
class GreeterServiceImpl final : public Greeter::Service {
 ...
 private:
  ...
  void initVec() {
    ...
    // TODO(zpzim): The tiling depends on what kind of join we have. We are
    // doing unnecessary work here
  ...
  bool sendVec(helloworld::SCAMPWork *reply) {
    // Fix decide if self join so B timeseries doesn't go or if B timeseries
    // data should go and not self join?
    //???????????????????????????????????????????
...

    {
      ...
      for (int i = 0; i < vec1.size(); i++) {
        if (vec1[i][5] == 0) {
          ...
          args->set_job_id(vec1[i][0]);  // this is idcnt
          ...
          args->set_distance_threshold(distance_threshold);
          args->set_computing_columns(computing_columns);
          args->set_computing_rows(computing_rows);
          args->set_precision_type(precision_type);
          args->set_profile_type(profile_type);
          args->set_keep_rows_separate(keep_rows_separate);
          args->set_is_aligned(is_aligned);
          args->set_window(window);
          args->set_has_b(has_b);
          //?????????????????????????????????
          // args->set_profile_a(profile_a);
          // args->set_profile_b(profile_b);

          std::cout << "Server sent idcnt: " << vec1[i][0]
                    << " size_a: " << args->timeseries_size_a()
                    << " size_b: " << args->timeseries_size_b()
                    << " Start time: " << timer << std::endl;
  ...
  Status SCAMPCombiner(ServerContext *context, const SCAMPArgs *request,
    ...
    printf("merging\n");
    MergeProfile(&global_profile_a, &tile_a, col_pos, width, global_a_lock,
                 &global_profile_b, &tile_b, row_pos, height, global_b_lock);
    printf("merging after\n");
    std::cout << "after merging print: "
              << global_profile_a.mutable_data()
                     ->Add()
                     ->mutable_uint64_value()
                     ->mutable_value()
              << std::endl;
    ...
    // std::transform(global_profile_a.data[0].cbegin(),
    // global_profile_a.data[0].cend(), outVector.begin(), [](const double&
    // in){return in;});

    // double chicken = global_profile_a.data[0].double_value.begin();

    // std::cout << "global_profile_a: " <<
    // global_profile_a.data[0].double_value.begin() << std::endl;
...
int main(int argc, char **argv) {
  ...
  num_tile_rows =
      ceil((Tb_h.size() - window + 1) / static_cast<double>(max_tile_size));
  num_tile_cols =
      ceil((Ta_h.size() - window + 1) / static_cast<double>(max_tile_size));
